...
def main():
    ...
    params_2 = torch.tensor(
        [0.9, 0.9, 0.9,   # scales
         1.0, 1.0,         # eps_1, eps_2
         0.0, 0.0, 0.0,    # small rotation
         0.0, 0.0, 0.0],   # centered at origin
        dtype=dtype,
        device=device,
    )
    print("Params: ", params_1.shape)
    params = torch.stack((params_1, params_2), dim=0)
    print("Params: ", params.shape)


    # # 1) Build grid
    min_xyz, max_xyz = global_bounding_box_for_all_SQs(params, margin=1.2)
    points, grid_shape = sample_grid_in_box(min_xyz, max_xyz, resolution=64,
                                            device=device, dtype=dtype)
    nx, ny, nz = grid_shape

    # 2) Evaluate field
    vals = multi_sq_implicit_union(points, params, signed=True)  # (N,)
    ...
    verts, faces, normals, values = measure.marching_cubes(grid, level=0.0)
    ...
    points_2 = sample_N_SQs_naive(params.tolist(), 32, 32)  # (N,3)
