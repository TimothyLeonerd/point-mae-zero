...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    base_model.cls_head_finetune = nn.Linear(in_features=768, out_features=config.model.cls_dim, bias=True)

    # Freeze all layers except `cls_head_finetune`
    for name, param in base_model.named_parameters():
        if "cls_head_finetune" not in name:
            param.requires_grad = False

    # Verify by printing which layers are trainable
    for name, param in base_model.named_parameters():
    ...
    frozen_weights = {}
    for name, param in base_model.named_parameters():
        if not param.requires_grad:  # Only save weights of frozen parameters
            frozen_weights[name] = param.clone().detach()
    for epoch in range(start_epoch, config.max_epoch + 1):
        # After training, check if frozen weights have changed
        for name, param in base_model.named_parameters():
            if not param.requires_grad:  # Only check frozen parameters
                if not torch.equal(frozen_weights[name], param):
                    print(f"Warning: Frozen parameter {name} has changed.")
